...
export default function CartDrawer() {
  ...
  return (
    <>
      <div
        className="fixed inset-0 bg-emerald-950/60 backdrop-blur-sm z-50 transition-opacity animate-fade-in"
        onClick={() => setIsCartOpen(false)}
      />
      <div
        className="fixed top-0 right-0 h-full w-full max-w-md glass-panel !rounded-none !border-y-0 !border-r-0 z-50 flex flex-col shadow-2xl animate-scale-in origin-right"
          ...
          {cartItems.length === 0 ? (
            ...
            cartItems.map((item, index) => (
              <div key={`${item.id}-${item.color}-${item.size}-${index}`} className="flex gap-4 p-4 rounded-2xl bg-white/5 border border-white/10 relative">
                <img src={item.image} alt={item.name} loading="lazy" decoding="async" className="w-20 h-28 object-cover rounded-xl" />
