  ...
  return (
    <section id="fabric-tech" className="py-24 px-6 relative z-10 w-full max-w-7xl mx-auto">
      <div className="glass-panel rounded-[2.5rem] p-8 md:p-16 overflow-hidden relative">
        ...
        <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-teal-500/10 rounded-full blur-[100px] transform -translate-y-1/2 translate-x-1/2"></div>
        <div className="absolute bottom-0 left-0 w-[400px] h-[400px] bg-blue-500/10 rounded-full blur-[80px] transform translate-y-1/2 -translate-x-1/4"></div>
        ...
        <div className="grid lg:grid-cols-2 gap-16 relative z-10">
          ...
          <div className="flex flex-col gap-6">
            {features.map((feature, index) => (
              <div 
                key={index}
                className="glass-card p-6 flex gap-6 items-start group animate-slide-up opacity-0"
              ...
              >
                <div className="shrink-0 p-4 rounded-2xl bg-white/5 border border-white/10 text-teal-300 group-hover:bg-teal-500/20 group-hover:border-teal-400/30 transition-colors duration-300 relative group/icon cursor-help">
                  {feature.icon}
                  {/* Tooltip */}
                  <div className="absolute -top-12 left-1/2 transform -translate-x-1/2 px-3 py-2 bg-emerald-900/95 backdrop-blur-md rounded-xl border border-white/20 opacity-0 group-hover/icon:opacity-100 group-hover/icon:-translate-y-1 transition-all duration-300 pointer-events-none whitespace-nowrap z-20 shadow-[0_0_15px_rgba(45,212,191,0.3)]">
                    <span className="text-xs font-medium text-white shadow-sm">{t(feature.benefitKey)}</span>
                    <div className="absolute -bottom-[5px] left-1/2 transform -translate-x-1/2 w-[10px] h-[10px] bg-emerald-900/95 border-r border-b border-white/20 rotate-45"></div>
                  </div>
                </div>
                <div>
